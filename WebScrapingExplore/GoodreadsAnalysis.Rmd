...
my_custom_theme <- theme_minimal() +
  theme(
    plot.background = element_rect(fill = "#fff2d4", color = NA),
    panel.background = element_rect(fill = "#fff2d4", color = NA),
    ...
    legend.background = element_rect(fill = "#fff2d4", color = NA),
    legend.key = element_rect(fill = "#fff2d4", color = NA),
...
We will import the Goodreads reviews dataset and the book genres dataset. We will also detect the language of the reviews and filter the dataset to keep only English reviews. Finally, we will join the reviews dataset with the book genres dataset to get the genres for each book.
...
We added a review id column to our dataset, removed rows with missing values, removed duplicate reviews, converted the review date to a date format, and removed special characters and converted the review text to lowercase.
...
Could just be we have more data from recent years book reviews.<nl>
...
ggplot(avg_review_length, aes(x = Review.Stars, y = Average.Length)) +
  geom_bar(stat = "identity", fill = darjeeling1[2], color="black") +
...
ggplot(top_n_books, aes(x = reorder(Book.Title, sentiment_score), y = sentiment_score)) +
  geom_col(fill = darjeeling1[2], alpa=0.8) +
