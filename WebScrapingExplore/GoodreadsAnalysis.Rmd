...
my_custom_theme <- theme_minimal() +
  theme(
    plot.background = element_rect(fill = "#fff8e8", color = NA),
    panel.background = element_rect(fill = "#fff8e8", color = NA),
    ...
    legend.background = element_rect(fill = "#fff8e8", color = NA),
    legend.key = element_rect(fill = "#fff8e8", color = NA),
...
full_reviews <- reviews %>%
  inner_join(book_genres, by = c("Book.Title" = "Book.Title"))  # Join reviews with book genres
...
```
Although the reviews for The Holy Bible: King James Version are mostly 1-star reviews, the average rating for the book on goodreads.com is 4.44. We filtered reviews for each book by most popular reviews so it is unclear why most of the popular reviews are 1-star reviews.
...
The top ten books with the highest total sentiment score across all reviews, as well the bottom ten books with the lowest total sentiment score across all reviews.
```{r}
...
print(top_highest_sentiment %>%
  select(Book.Title, Genres, sentiment, avg_rating))
...
```
Upon investigating the genres for the book Night, we see that the book is a historical biography about the holocaust which explains the low sentiment score for all the reviews.
...
```
```{r}
# Filter for the top-rated book
top_rated_book <- reviews %>%
  group_by(Book.Title) %>%
  summarise(average_rating = mean(Review.Stars, na.rm = TRUE)) %>%
  top_n(1, wt = average_rating)

# Filter reviews for that top-rated book
top_book_reviews <- reviews %>%
  filter(Book.Title == top_rated_book$Book.Title[1])

# Assuming you have sentiment scores already computed in 'sentiment_scores'
# Merge or filter your sentiment_scores dataframe for the top-rated book reviews
top_book_sentiment <- sentiment_scores %>%
  filter(Book.Title == top_rated_book$Book.Title[1])

# Plot sentiment over time for the top-rated book
ggplot(top_book_sentiment, aes(x = Review.Date, y = sentiment_score)) +
  geom_smooth(method = "loess", se = FALSE, color = darjeeling1[4]) +
  labs(title = paste("Sentiment Score Trend Over Time for", top_rated_book$Book.Title[1]),
       x = "Review Date",
       y = "Sentiment Score") +
  theme(plot.title = element_text(hjust = 0.5))
```
...
avg_sentiment_per_year <- top_book_sentiment %>%
...
ggplot(avg_sentiment_per_year, aes(x = year, y = avg_sentiment_score)) +
  geom_col(fill = darjeeling1[4]) +  # Use geom_col to create a bar plot
  labs(title = paste("Average Sentiment Score per Year for", top_rated_book$Book.Title[1]),
       x = "Year",
       y = "Average Sentiment Score") +
  theme(plot.title = element_text(hjust = 0.5))
```
